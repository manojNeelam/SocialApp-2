...
- (void)buttonTapped:(SCLButton *)btn
{
    ...
    if (btn.actionType == SCLBlockdict)
    {
        if (btn.actionBlock)
            btn.actionBlock();
    }
...
- (SCLAlertViewResponder *)showTitle:(UIViewController *)vc image:(UIImage *)image color:(UIColor *)color title:(NSString *)title subTitle:(NSString *)subTitle duration:(NSTimeInterval)duration completeText:(NSString *)completeText style:(SCLAlertViewStyle)style
{
    ...
    iconImage = [UIImage imageNamed:@"errorLoction@1x"];
    // Icon style
    switch (style)
    {
        case Success:
        ...
        case Warning:
            viewColor = CONTACTS_BG_COLOUR;//UIColorFromHEX(0xFFD110);
        ...
        case Edit:
            viewColor = SCREAM_BG_COLOUR;//UIColorFromHEX(0xA429FF);
            iconImage = SCLAlertViewStyleKit.imageOfEdit;
            break;
            
        case Waiting:
            viewColor = ALERTMESSAGE_BG_COLOR;
            break;
